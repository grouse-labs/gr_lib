...
function ped.catch(netID, options)
  local obj = {}
  obj.netID = netID
  obj.entity = NetworkGetNetworkIdFromEntity(netID)
  obj.options = {
    data = {},
    ranges = {},
    combat_ai = {},
  ...
  peds[netID] = setmetatable(obj, {__index = ped})
  return peds[netID]
end
...
RegisterNetEvent(string.format(EVENT, RESOURCE, CONTEXT, 'ped_initialise'), ped.catch)
RegisterNetEvent(string.format(EVENT, RESOURCE, CONTEXT, 'ped_destroy'), ped.remove)
