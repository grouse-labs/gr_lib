...
---@return boolean is_child `child` is a child of `parent`.
local function is_parent(parent, child) return not not child:match('^'..parent..'%-') end

---@param array any[]
---@param length integer
---@return any[] reversed
local function reverse_array(array, length)
  local reversed = {}
  local lim = length or #array
  for i = lim, 1, -1 do reversed[(lim - i) + 1] = array[i] end
  return reversed
end

---@param locale_code string The locale code.
---@return string[] ancestors ancestry of the locale code.
local function ancestry(locale_code)
  ...
  return reverse_array(result, length)
end
...
function locale.translate(key, data)
  ...
  data = data or {}
  if not next(storage) then locale.loadfile() end
